...
function getGuestTokenRefreshTiming(currentGuestToken) {
  console.log("TOKEN ", currentGuestToken)
  const context = Buffer.from(currentGuestToken.split('.')[1], 'base64').toString()
  const parsedJwt = JSON.parse(context); // if exp is int, it is in seconds, but Date() takes milliseconds
